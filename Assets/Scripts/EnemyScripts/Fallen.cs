...
public class Fallen : CharacterGeneral
{

    public float f_Distance; //Player와 Fallen과의 거리

    public bool b_IsSearch = false; //Player 탐색 여부

    public GameObject FallenPrefab; //Fallen 프리팹
    public GameObject Target; //공격할 타겟
    public GameObject Fallen_Muzzle; //Fallen 공격 시작점
    public GameObject EnemyUiPrefab; //적 UI 프리팹

    public Sprite FallenBulletImage; //Fallen 총알 이미지
    //public Sprite FallenBombImage; //Fallen 폭탄 이미지(미적용)

    public GeneralInitialize.GunParameter cur_EnemyWeapon; //적 무기 정보

    Vector3 v_TargetPosition = new Vector3(); //타겟 정중앙 위치 Vector3 값
    Vector3 v_Accurate = new Vector3(0, 0.5f, 0); //Target의 정중앙을 조준하기 위한 벡터
    ...
    void Start()
    {
        ...
        FallenBulletImage = tempEnemyWeapon.BulletImage;
    ...
    void FireBullet_First_Pattern()
    {
        Debug.Log("[Fallen] FireBullet_First_Pattern called");
        if (Fallen_Muzzle)
        {
            Vector3 v_muzzle = Fallen_Muzzle.transform.position;
            Vector3 v_bulletSpeed = (v_TargetPosition - Fallen_Muzzle.transform.position).normalized * cur_EnemyWeapon.f_BulletSpeed;

            this.photonView.RPC("FireBullet_First_Pattern_Network", PhotonTargets.All, v_muzzle, v_bulletSpeed);
    ...
    void FireBullet_Second_Pattern()
    {
        ...
        {
            Vector3 v_muzzle = Fallen_Muzzle.transform.position;
            Vector3 v_bulletSpeed = (Fallen_Muzzle.transform.position - (Target.transform.position + v_Accurate)).normalized * 2.0f;

            this.photonView.RPC("FireBullet_Second_Pattern_Network", PhotonTargets.All, v_muzzle, v_bulletSpeed);
            this.photonView.RPC("FireAnimationNetwork", PhotonTargets.Others);
        }
        else
        {
    ...
    void FireBullet_Third_Pattern()
    {
        Debug.Log("[Fallen] FireBullet_First_Pattern called");
        if (Fallen_Muzzle)
        {
            Vector3 v_muzzle = Fallen_Muzzle.transform.position;
            Vector3 v_bulletSpeed = (Fallen_Muzzle.transform.position - (Target.transform.position + v_Accurate)).normalized * cur_EnemyWeapon.f_BulletSpeed;

            this.photonView.RPC("FireBullet_Third_Pattern_Network", PhotonTargets.All, v_muzzle, v_bulletSpeed);
            this.photonView.RPC("FireAnimationNetwork", PhotonTargets.Others);
        }
        else
        {
    ...
    [PunRPC]
    //플레이어를 향해 3개의 총알을 부채꼴로 발사합니다.
    void FireBullet_First_Pattern_Network(Vector3 muzzlePos, Vector3 bulletSpeed)
    {
        ...
        BulletGeneral temp_bullet2 = bullet1.GetComponent<BulletGeneral>();
        BulletGeneral temp_bullet3 = bullet1.GetComponent<BulletGeneral>();
        ...
        bullet1.GetComponent<Rigidbody2D>().velocity = ((v_TargetPosition + v_Accurate) - muzzlePos).normalized * this.cur_EnemyWeapon.f_BulletSpeed;
        bullet2.GetComponent<Rigidbody2D>().velocity = (v_TargetPosition - muzzlePos).normalized * this.cur_EnemyWeapon.f_BulletSpeed;
        bullet3.GetComponent<Rigidbody2D>().velocity = (Target.transform.position - muzzlePos).normalized * this.cur_EnemyWeapon.f_BulletSpeed;
    }
    ...
    void FireBullet_Second_Pattern_Network(Vector3 muzzlePos, Vector3 bulletSpeed)
    {
        ...
        bomb.GetComponent<Rigidbody2D>().velocity = (v_TargetPosition - muzzlePos).normalized * 2.0f;
    }

    [PunRPC]
    //십자선상 네 방향으로 총알을 발사합니다.
    void FireBullet_Third_Pattern_Network(Vector3 muzzlePos, Vector3 bulletSpeed)
    {
        Vector3 front = new Vector3(1, 0, 0);
        Vector3 Up = new Vector3(0, 1, 0);
        Debug.Log("[Fallen] FireBullet_Third_Pattern_Network called");
        GameObject bullet1 = Instantiate(this.g_Bullet, muzzlePos, Quaternion.identity);
        GameObject bullet2 = Instantiate(this.g_Bullet, muzzlePos, Quaternion.identity);
        GameObject bullet3 = Instantiate(this.g_Bullet, muzzlePos, Quaternion.identity);
        ...
        bullet3.GetComponent<Rigidbody2D>().velocity = (muzzlePos - (muzzlePos + Up)).normalized * this.cur_EnemyWeapon.f_BulletSpeed;
        bullet4.GetComponent<Rigidbody2D>().velocity = ((muzzlePos + Up) - muzzlePos).normalized * this.cur_EnemyWeapon.f_BulletSpeed;
    }
    ...
    protected override void WeaponSpineControl(bool _b_EnemyFired)
    {
        ...
        if (b_IsSearch == true)
        {
            ...
            {
                FireBullet_Second_Pattern();
                spine_GunAnim.state.SetAnimation(0, "Shoot", false);
            }
        }
        /*
        //First Pattern
        if (b_IsSearch == true && n_hp > 700)
        {
            if (!_b_EnemyFired && Target.GetComponent<CharacterGeneral>().n_hp > 0)
            {
                FireBullet_First_Pattern();
                spine_GunAnim.state.SetAnimation(0, "Shoot", false);
            }
        }
        //Second Pattern
        if (b_IsSearch == true && (n_hp > 400 && n_hp < 700))
        {
            if (!_b_EnemyFired && Target.GetComponent<CharacterGeneral>().n_hp > 0)
            {
                FireBullet_Second_Pattern();
        ...
        //Third Pattern
        if (b_IsSearch == true && n_hp < 400)
        {
            if (!_b_EnemyFired && Target.GetComponent<CharacterGeneral>().n_hp > 0)
            {
                FireBullet_Third_Pattern();
                spine_GunAnim.state.SetAnimation(0, "Shoot", false);
            }
        }
        */
